...
//Methods to "eat" the current token
	...
	/**
	 * Throws an exception if the current token is not a comparison operator.
	 * Otherwise, returns that comparison operator and moves to the next token.
	 * 
	 * @return the string value of the comparison operator
	 */
	public String eatOpr() {		
		if (!matchOpr())
			throw new BadSyntaxException();

		// Check the first character of the operator
		char oprFirstChar = (char) tok.ttype;
		// Check the second character of the operator
		nextToken();
		char oprSecondChar = (char) tok.ttype;
