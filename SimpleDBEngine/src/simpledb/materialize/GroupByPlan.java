...
public class GroupByPlan implements Plan {
	...
	public GroupByPlan(Transaction tx, Plan p, List<String> groupfields, List<AggregationFn> aggfns) {
		...
		for (AggregationFn fn : aggfns)
			if (fn instanceof AvgFn) {
				sch.addFloatField(fn.fieldName());
			} else {
	...
	public void printPlan() {
		String stmt = "Group By Plan used ";
		if (!groupfields.isEmpty()) {
			stmt += "to group fields of ";
			for (String groupField : groupfields) {
				stmt += groupField + ", ";			
			}
			stmt = stmt.substring(0, stmt.length() - 2); // remove final comma
		}
		if (!aggfns.isEmpty()) {
			if (!groupfields.isEmpty()) {
				stmt += " and ";
			}
			stmt += "to aggregate fields of ";
			for (AggregationFn aggFn : aggfns) {
				stmt += aggFn.fieldName() + ", ";
			}
			stmt = stmt.substring(0, stmt.length() - 2); // remove final comma
		}
