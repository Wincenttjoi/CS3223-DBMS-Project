...
public class IndexJoinTest {
	public static void main(String[] args) {
      ...
      Transaction tx = db.newTx();
		...
		IndexInfo majorIdx = indexes.get("majorid");
		...
		useIndexManually(deptplan, studentplan, majorIdx, "did", "<");		
		useIndexScan(deptplan, studentplan, majorIdx, "did", "<");

		tx.commit();
	}

	private static void useIndexManually(Plan p1, Plan p2, IndexInfo ii, String joinfield, String opr) {
		...
		while (s1.next()) {
			...
			while (idx.next()) {
				...
				System.out.println(s1.getString("dname") + s1.getVal("did") + s2.getVal("majorid") +  s2.getVal("sname"));
			}
		}
		idx.close();
		s1.close();
	...
	private static void useIndexScan(Plan p1, Plan p2, IndexInfo ii, String joinfield, String opr) {
		// Open an index join scan on the table.
		Plan idxplan = new IndexJoinPlan(p1, p2, ii, joinfield, opr);
		Scan s = idxplan.open();

		while (s.next()) {
			System.out.println(s.getString("dname") + s.getVal("did") + s.getVal("majorid") +  s.getVal("sname"));
		}
		s.close();
	}
}
