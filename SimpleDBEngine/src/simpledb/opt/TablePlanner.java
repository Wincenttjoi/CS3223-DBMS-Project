...
class TablePlanner {
	...
	public Plan makeJoinPlan(Plan current, JoinAlgoSelector joinAlgoSelected) {
		...
		if (joinAlgoSelected == null) {
			...
			List<Plan> plans = new ArrayList<Plan>(
					Arrays.asList(indexJoinPlan, mergeJoinPlan, nestedJoinPlan, hashJoinPlan));

			Stream<Plan> plansStream = plans.stream().filter((p1) -> p1 != null)
		...
		for (String fldname : indexes.keySet()) {
			...
			if (outerfield != null && currsch.hasField(outerfield)) {
				if (ii.supportsRangeSearch(opr)) {
					...
					joinTermsToRemove[JoinAlgoSelector.INDEXJOIN_PLAN.ordinal()] = new Term(new Expression(fldname),
							new Expression(outerfield), opr);
					return p;
				} else {
					System.out.println(tab + "Indexjoin failed: " + opr + " not supported by " + ii.getFieldName()
							+ ", using productjoin instead");
				}
			}
		}
	...
	private Plan makeMergeJoin(Plan current, Schema currsch) {
		...
		joinTermsToRemove[JoinAlgoSelector.MERGEJOIN_PLAN.ordinal()] = new Term(new Expression(joinValLHS),
				new Expression(joinValRHS), joinTerm.getOpr());
		return p;
	}
	...
	private Plan makeNestedJoin(Plan current, Schema currsch) {
		...
		joinTermsToRemove[JoinAlgoSelector.NESTEDJOIN_PLAN.ordinal()] = new Term(new Expression(joinValLHS),
				new Expression(joinValRHS), opr);
		return p;
	}
	...
	private Plan makeHashJoin(Plan current, Schema currsch) {
		...
		joinTermsToRemove[JoinAlgoSelector.HASHJOIN_PLAN.ordinal()] = new Term(new Expression(joinValLHS),
				new Expression(joinValRHS), opr);
		return p;
	}
	...
	/**
	 * Allows a new plan to be created without the joinTerm used by the selected
	 * join algorithm from the predicate.
	 * 
	 */
